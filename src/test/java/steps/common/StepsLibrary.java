...
public class StepsLibrary {
	...
	public static void clickAndSendKeys(String text, String loc)  {
		try{
			CommonStep.click(loc);
			Thread.sleep(6000);
			new Actions(new WebDriverTestBase().getDriver()).sendKeys(text).build().perform();
		}catch (Exception e) {
			System.out.println("Exception Occured during clickAndSendKeys : "+e);
		}
	}

	public static void clickAndSendEncryptedKeys(String text, String loc) {
		try{
			byte[] decoded = Base64.getDecoder().decode(text);
			CommonStep.click(loc);
			Thread.sleep(6000);
			new Actions(new WebDriverTestBase().getDriver()).sendKeys(new String(decoded)).build().perform();
		}catch (Exception e) {
			System.out.println("Exception Occured during clickAndSendEncryptedKeys : "+e);
		}
	}
}
